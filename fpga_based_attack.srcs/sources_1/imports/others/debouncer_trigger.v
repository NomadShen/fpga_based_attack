...
module debouncer_trigger(
    input clk,
    input A,
    output B
);
    parameter COUNT_MAX   = 3, // number of clock cycles the input must be stable for before the output will transition.
			  COUNT_WIDTH = 2;  // ceil(log2(COUNT_MAX)) suggested, number of bits for the delay counter.
