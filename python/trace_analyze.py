...
# point of interest
with open(data_folder+"/profile_0.csv", mode="r", encoding="utf-8-sig") as f:
    reader = csv.reader(f)
    header = next(reader)
    radix = next(reader)
...
for i in trange(trace_num):
    ...
    if corr_sorted[-2] == corr[correct_ans]:
        cnt_sr2 += 1
    # significance
    corr_other = np.delete(corr,correct_ans)
    sig += corr[correct_ans]-max(corr_other)

    # confidence
    confidence = corr_sorted[-1] - corr_sorted[-2]

    if corr.argmax(axis=0) == correct_ans:    
        c_confidence.append(confidence)
    else:
        w_confidence.append(confidence)
...
print("avg significance\t",sig/trace_num)